import type { ProjectDocument } from '../domain/model'
import { CURRENT_SCHEMA_VERSION, DEFAULT_COMPONENT_LAYOUT } from '../domain/model'

export const sampleProject: ProjectDocument = {
  schemaVersion: CURRENT_SCHEMA_VERSION,
  revision: 1,
  project: {
    id: 'proj-1',
    name: 'User Management',
    screenIds: ['screen-list', 'screen-edit'],
  },
  screens: {
    'screen-list': {
      id: 'screen-list',
      name: 'User List',
      route: '/users',
      rootComponentId: 'comp-list-page',
      modalComponentIds: [],
      defaultStateId: 'state-list-default',
      stateIds: ['state-list-default', 'state-list-loading'],
      eventIds: [],
    },
    'screen-edit': {
      id: 'screen-edit',
      name: 'Edit User',
      route: '/users/:id/edit',
      rootComponentId: 'comp-edit-page',
      modalComponentIds: ['comp-delete-modal'],
      defaultStateId: 'state-edit-default',
      stateIds: [
        'state-edit-default',
        'state-edit-saving',
        'state-edit-success',
        'state-edit-error',
      ],
      eventIds: ['event-submit'],
    },
  },
  components: {
    // User list screen
    'comp-list-page': {
      id: 'comp-list-page',
      screenId: 'screen-list',
      parentId: null,
      childIds: ['comp-list-section'],
      kind: 'page',
      common: { description: '', visible: true, enabled: true },
      config: { kind: 'page', ...DEFAULT_COMPONENT_LAYOUT },
    },
    'comp-list-section': {
      id: 'comp-list-section',
      screenId: 'screen-list',
      parentId: 'comp-list-page',
      childIds: ['comp-list-title', 'comp-list-grid'],
      kind: 'container',
      common: { description: '', visible: true, enabled: true },
      config: { kind: 'container', ...DEFAULT_COMPONENT_LAYOUT },
    },
    'comp-list-title': {
      id: 'comp-list-title',
      screenId: 'screen-list',
      parentId: 'comp-list-section',
      childIds: [],
      kind: 'text',
      common: { description: '', visible: true, enabled: true },
      config: { kind: 'text', text: 'User List', style: 'heading1' },
    },
    'comp-list-grid': {
      id: 'comp-list-grid',
      screenId: 'screen-list',
      parentId: 'comp-list-section',
      childIds: ['comp-list-active', 'comp-list-invited'],
      kind: 'container',
      common: { description: 'User summary', visible: true, enabled: true },
      config: {
        kind: 'container',
        ...DEFAULT_COMPONENT_LAYOUT,
        layout: 'grid',
        columns: 2,
        gap: 'sm',
      },
    },
    'comp-list-active': {
      id: 'comp-list-active',
      screenId: 'screen-list',
      parentId: 'comp-list-grid',
      childIds: [],
      kind: 'text',
      common: { description: '', visible: true, enabled: true },
      config: { kind: 'text', text: 'Active users: 24', style: 'body' },
    },
    'comp-list-invited': {
      id: 'comp-list-invited',
      screenId: 'screen-list',
      parentId: 'comp-list-grid',
      childIds: [],
      kind: 'text',
      common: { description: '', visible: true, enabled: true },
      config: { kind: 'text', text: 'Pending invitations: 3', style: 'body' },
    },
    // Edit user screen
    'comp-edit-page': {
      id: 'comp-edit-page',
      screenId: 'screen-edit',
      parentId: null,
      childIds: ['comp-edit-section'],
      kind: 'page',
      common: { description: '', visible: true, enabled: true },
      config: { kind: 'page', ...DEFAULT_COMPONENT_LAYOUT },
    },
    'comp-edit-section': {
      id: 'comp-edit-section',
      screenId: 'screen-edit',
      parentId: 'comp-edit-page',
      childIds: ['comp-edit-title', 'comp-name-input', 'comp-email-input', 'comp-role-select', 'comp-status-alert', 'comp-actions'],
      kind: 'container',
      common: { description: '', visible: true, enabled: true },
      config: { kind: 'container', ...DEFAULT_COMPONENT_LAYOUT },
    },
    'comp-edit-title': {
      id: 'comp-edit-title',
      screenId: 'screen-edit',
      parentId: 'comp-edit-section',
      childIds: [],
      kind: 'text',
      common: { description: '', visible: true, enabled: true },
      config: { kind: 'text', text: 'User Details', style: 'heading2' },
    },
    'comp-name-input': {
      id: 'comp-name-input',
      screenId: 'screen-edit',
      parentId: 'comp-edit-section',
      childIds: [],
      kind: 'textInput',
      common: { description: '', visible: true, enabled: true },
      config: {
        kind: 'textInput',
        fieldKey: 'name',
        label: 'Name',
        inputType: 'text',
        required: true,
        placeholder: 'e.g. Alex Smith',
        defaultValue: '',
        validationRules: [
          { id: 'vr-1', type: 'required', message: 'Name is required' },
          { id: 'vr-2', type: 'maxLength', value: 50, message: 'Enter no more than 50 characters' },
        ],
      },
    },
    'comp-email-input': {
      id: 'comp-email-input',
      screenId: 'screen-edit',
      parentId: 'comp-edit-section',
      childIds: [],
      kind: 'textInput',
      common: { description: '', visible: true, enabled: true },
      config: {
        kind: 'textInput',
        fieldKey: 'email',
        label: 'Email address',
        inputType: 'email',
        required: true,
        placeholder: 'example@example.com',
        defaultValue: '',
        validationRules: [
          { id: 'vr-3', type: 'required', message: 'Email address is required' },
          { id: 'vr-4', type: 'email', message: 'Enter a valid email address' },
        ],
      },
    },
    'comp-role-select': {
      id: 'comp-role-select',
      screenId: 'screen-edit',
      parentId: 'comp-edit-section',
      childIds: [],
      kind: 'select',
      common: { description: '', visible: true, enabled: true },
      config: {
        kind: 'select',
        fieldKey: 'role',
        label: 'Role',
        required: true,
        options: [
          { value: 'member', label: 'Member' },
          { value: 'admin', label: 'Administrator' },
        ],
        defaultValue: 'member',
      },
    },
    'comp-actions': {
      id: 'comp-actions',
      screenId: 'screen-edit',
      parentId: 'comp-edit-section',
      childIds: ['comp-cancel-btn', 'comp-save-btn'],
      kind: 'container',
      common: { description: '', visible: true, enabled: true },
      config: {
        kind: 'container',
        ...DEFAULT_COMPONENT_LAYOUT,
        layout: 'horizontal',
        gap: 'sm',
        justify: 'end',
        align: 'center',
        wrap: true,
      },
    },
    'comp-status-alert': {
      id: 'comp-status-alert',
      screenId: 'screen-edit',
      parentId: 'comp-edit-section',
      childIds: [],
      kind: 'alert',
      common: { description: 'Save result', visible: false, enabled: true },
      config: {
        kind: 'alert',
        tone: 'success',
        message: 'User saved successfully.',
      },
    },
    'comp-cancel-btn': {
      id: 'comp-cancel-btn',
      screenId: 'screen-edit',
      parentId: 'comp-actions',
      childIds: [],
      kind: 'button',
      common: { description: '', visible: true, enabled: true },
      config: {
        kind: 'button',
        label: 'Cancel',
        variant: 'secondary',
        eventId: null,
        confirmationMessage: null,
        preventDoubleSubmit: false,
      },
    },
    'comp-save-btn': {
      id: 'comp-save-btn',
      screenId: 'screen-edit',
      parentId: 'comp-actions',
      childIds: [],
      kind: 'button',
      common: { description: '', visible: true, enabled: true },
      config: {
        kind: 'button',
        label: 'Save',
        variant: 'primary',
        eventId: 'event-submit',
        confirmationMessage: null,
        preventDoubleSubmit: false,
      },
    },
    'comp-delete-modal': {
      id: 'comp-delete-modal',
      screenId: 'screen-edit',
      parentId: null,
      childIds: ['comp-delete-modal-section'],
      kind: 'modal',
      common: { description: 'Delete user confirmation', visible: true, enabled: true },
      config: {
        kind: 'modal',
        ...DEFAULT_COMPONENT_LAYOUT,
        gap: 'sm',
      },
    },
    'comp-delete-modal-section': {
      id: 'comp-delete-modal-section',
      screenId: 'screen-edit',
      parentId: 'comp-delete-modal',
      childIds: ['comp-delete-title', 'comp-delete-warning', 'comp-delete-actions'],
      kind: 'container',
      common: { description: '', visible: true, enabled: true },
      config: { kind: 'container', ...DEFAULT_COMPONENT_LAYOUT, gap: 'sm' },
    },
    'comp-delete-title': {
      id: 'comp-delete-title',
      screenId: 'screen-edit',
      parentId: 'comp-delete-modal-section',
      childIds: [],
      kind: 'text',
      common: { description: '', visible: true, enabled: true },
      config: { kind: 'text', text: 'Delete user?', style: 'heading2' },
    },
    'comp-delete-warning': {
      id: 'comp-delete-warning',
      screenId: 'screen-edit',
      parentId: 'comp-delete-modal-section',
      childIds: [],
      kind: 'text',
      common: { description: 'Deletion warning', visible: true, enabled: true },
      config: {
        kind: 'text',
        text: 'This action cannot be undone.',
        style: 'body',
      },
    },
    'comp-delete-actions': {
      id: 'comp-delete-actions',
      screenId: 'screen-edit',
      parentId: 'comp-delete-modal-section',
      childIds: ['comp-delete-cancel', 'comp-delete-confirm'],
      kind: 'container',
      common: { description: '', visible: true, enabled: true },
      config: {
        kind: 'container',
        ...DEFAULT_COMPONENT_LAYOUT,
        layout: 'horizontal',
        gap: 'sm',
        justify: 'end',
        align: 'center',
        wrap: true,
      },
    },
    'comp-delete-cancel': {
      id: 'comp-delete-cancel',
      screenId: 'screen-edit',
      parentId: 'comp-delete-actions',
      childIds: [],
      kind: 'button',
      common: { description: '', visible: true, enabled: true },
      config: {
        kind: 'button',
        label: 'Cancel',
        variant: 'secondary',
        eventId: null,
        confirmationMessage: null,
        preventDoubleSubmit: false,
      },
    },
    'comp-delete-confirm': {
      id: 'comp-delete-confirm',
      screenId: 'screen-edit',
      parentId: 'comp-delete-actions',
      childIds: [],
      kind: 'button',
      common: { description: '', visible: true, enabled: true },
      config: {
        kind: 'button',
        label: 'Delete',
        variant: 'danger',
        eventId: null,
        confirmationMessage: 'Delete this user?',
        preventDoubleSubmit: true,
      },
    },
  },
  screenStates: {
    'state-list-default': {
      id: 'state-list-default',
      screenId: 'screen-list',
      name: 'Default',
      description: '',
      componentOverrides: {},
    },
    'state-list-loading': {
      id: 'state-list-loading',
      screenId: 'screen-list',
      name: 'Loading',
      description: 'Loading the user list',
      componentOverrides: {
        'comp-list-title': { text: 'Loading users...', enabled: false },
      },
    },
    'state-edit-default': {
      id: 'state-edit-default',
      screenId: 'screen-edit',
      name: 'Default',
      description: '',
      componentOverrides: {},
    },
    'state-edit-saving': {
      id: 'state-edit-saving',
      screenId: 'screen-edit',
      name: 'Saving',
      description: 'Sending data to the API',
      componentOverrides: {
        'comp-save-btn': { enabled: false },
        'comp-cancel-btn': { enabled: false },
      },
    },
    'state-edit-success': {
      id: 'state-edit-success',
      screenId: 'screen-edit',
      name: 'Success',
      description: 'The user was saved',
      componentOverrides: {
        'comp-status-alert': { visible: true, message: 'User saved successfully.' },
        'comp-role-select': { value: 'admin' },
        'comp-actions': { visible: false },
      },
    },
    'state-edit-error': {
      id: 'state-edit-error',
      screenId: 'screen-edit',
      name: 'Error',
      description: 'The save request failed',
      componentOverrides: {
        'comp-status-alert': { visible: true, message: 'Could not save the user.' },
      },
    },
  },
  events: {
    'event-submit': {
      id: 'event-submit',
      screenId: 'screen-edit',
      name: 'Save button click',
      trigger: { type: 'click', componentId: 'comp-save-btn' },
      actions: [
        { type: 'setState', stateId: 'state-edit-saving' },
        { type: 'callApi', apiOperationId: 'api-save-user' },
      ],
    },
  },
  apiOperations: {
    'api-save-user': {
      id: 'api-save-user',
      screenId: 'screen-edit',
      name: 'Save user',
      method: 'PUT',
      path: '/api/users/{id}',
      requestBindings: [
        { componentId: 'comp-name-input', targetPath: 'body.name' },
        { componentId: 'comp-email-input', targetPath: 'body.email' },
        { componentId: 'comp-role-select', targetPath: 'body.role' },
      ],
      successStateId: 'state-edit-success',
      errorStateId: 'state-edit-error',
    },
  },
}
