...
WebMCP Challengeでは、**再利用可能なUIコンポーネントを組み合わせてワイヤーフレームを作り、同じ構造化モデルで画面状態・イベント・API連携・画面遷移を一体的に管理するWebアプリ**を作成する。
...
4. エージェントの変更が同じUIへ現れ、人間が反映・破棄を判断できる
5. lock解除後の人間編集や破棄記録を、エージェントが次の操作に利用する
...
- イベント、API、バリデーション
- エージェントの変更セット
- 反映、破棄、反映済みtransactionのUndo

MVPでは自由配置ではなく、パレットからのdrag追加と選択componentのcontext menu追加、同一親内の並び替え、container間移動をtree/canvas共通のcommandとして提供する。drag & dropにはpointer、touch、keyboard sensorを用意する。
...
- 未保存の編集内容
- 反映前のエージェント変更セット
- 直近に破棄したchange setのsummaryとoperation概要
...
> Compose screens from semantic UI components, then keep wireframes, states, events, API bindings, and screen flows aligned in the same source of truth.
...
| 評価項目 | このプロダクトで示す内容 |
| --- | --- |
| WebMCP Leverage | 選択中の要素、effectiveな画面一式、状態、未反映diff、破棄記録を型付きツールで共有し、人間との往復に利用する |
| Execution | 手動での画面構築、状態・event・API編集、AI変更preview、review lock、反映・破棄まで一貫して動作する |
| Potential Impact | デザインと仕様の乖離、仕様漏れ、職種間の認識差という具体的問題を解決する |
| Creativity & Ambition | 汎用AIワイヤーフレームではなく、画面構造・状態・振る舞い・API・遷移フローを同一モデルに統合する |

最大の失敗パターンは、WebMCPツールが単なるYAML編集APIになることである。デモでは必ず、ページ上の現在選択、未反映の変更セット、人間の反映・破棄、lock解除後の通常編集をエージェントが再読する場面を見せる。
