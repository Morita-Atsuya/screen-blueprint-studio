# Screen Spec Studio プロダクト方針・引き継ぎ
...
仮称は **Screen Spec Studio** とする。名称は後から変更してよい。
...
3. 人間とエージェントが同じ画面、選択、未保存ドラフト、変更案を共有する
4. エージェントの変更が同じUIへ現れ、人間が修正・承認・却下できる
5. 人間の修正や却下を、エージェントが次の操作に利用する
6. 名前付き・型付きツールが、座標クリックより安全で意味のある操作になる

Screen Spec Studioでは、ワイヤーフレーム上の選択、編集中の仕様、表示中の状態、診断結果、未承認の変更案がブラウザ内に存在するため、この条件を満たせる。
...
初期MVPではドラッグ操作に固執しない。クリックによる追加、上下移動、セクション変更でも、共同編集の価値と仕様生成は実証できる。

## 7. WebMCPを使う必然性

通常のMCPでも、保存済みのJSONやYAMLを読み書きできる。しかし、Screen Spec Studioで重要なのは次のページ内状態である。
...
- Screen Spec Studio: 実装前の画面構造と振る舞いを、視覚的かつ検証可能な仕様として定義する
...
| 評価項目 | このプロダクトで示す内容 |
| --- | --- |
| WebMCP Leverage | 選択中の要素、未保存仕様、状態、診断、却下履歴を型付きツールで共有し、人間との往復に利用する |
| Execution | 手動でも画面を構築でき、AI提案、レビュー、仕様出力まで一貫した体験を完成させる |
| Potential Impact | デザインと仕様の乖離、仕様漏れ、職種間の認識差という具体的問題を解決する |
| Creativity & Ambition | 汎用AIワイヤーフレームではなく、デザイン・振る舞い・API・テストを同一モデルに統合する |

最大の失敗パターンは、WebMCPツールが単なるYAML編集APIになることである。デモでは必ず、ページ上の現在選択、未承認変更、人間の修正をエージェントが再利用する場面を見せる。

## 13. 実装方針

- Challenge用リポジトリでゼロから独立実装する
- 初期スターターのメモアプリは置き換える
- React、TypeScript、Viteなどでクライアント中心に構築する想定だが、技術選定は設計時に確定する
- データモデルと状態管理を先に設計し、キャンバスと仕様書を同じモデルから描画する
- 人間向け操作とWebMCPツールで同じ更新関数・検証処理を呼ぶ
- 変更は直接確定させず、エージェント提案をchange setとしてレビューできるようにする
