...
Screen Blueprint Studioでは、ワイヤーフレーム上の選択、編集中の仕様、表示中の状態、未承認の変更案がブラウザ内に存在するため、この条件を満たせる。
...
| 評価項目 | このプロダクトで示す内容 |
| --- | --- |
| WebMCP Leverage | 選択中の要素、未保存仕様、状態、未承認の変更案、人間による修正を型付きツールで共有し、人間との往復に利用する |
...
- エージェント提案はchange setとしてレビューし、人間だけが承認または却下する
